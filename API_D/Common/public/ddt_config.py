...
def mk_test_name(name, value, index=0, name_fmt=TestNameFormat.DEFAULT):
    ...
    # 添加了对字典数据的处理。
    # if name_fmt is TestNameFormat.INDEX_ONLY or not is_trivial(value) is not dict:
        return "{0}_{1}".format(name, index)
    # 如果数据是字典，则获取字典当中的api_name对应的值，加到测试用例名称中。
    # if type(value) is dict:
    #     try:
    #         value = value["Title"]  # case_name作为value值，通过key获取标题的值
    #     except:
    #         return "{0}_{1}".format(name, index)
    try:
        value = str(value[3])   # 索引3为测试标题在列表中的位置
