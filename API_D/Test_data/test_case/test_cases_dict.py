...
from API_D.Common.public.do_excel import DoExcel
...
class testCases(unittest.TestCase):
    '''这是一个发送http请求的类'''
    t=DoExcel(project_path.excel_path).read_data('test')
    @data(*t)
    def test_cases(self, data_dict):
        '''这是一个用来测试注册登录的模块'''
        # headers={'user-agent':'Mozilla/5.0','Connection':'keep-alive'}
        CaseId = data_dict['CaseId']
        Module = data_dict['Module']
        URL = data_dict['URL']
        Method = data_dict['Method']
        Params = data_dict['Params']
        Sql = data_dict['Sql']
        ExpectedResult = data_dict['ExpectedResult']
        ...
        if ExpectedResult.find('money') != -1:
            ...
            # print(eval(Params)['amount'])
            # print(getattr(getdata,'MONEY'))
            # print(str(int(eval(Params)['amount'])+getattr(getdata,'MONEY')))
        else:
        ...
        if ExpectedResult.find('yue') != -1:
            ...
            ExpectedResult=ExpectedResult
        DoExcel(project_path.excel_path).write_back('test',CaseId+1,9,res.text)
        '''将实际结果写入表格中'''
        try:
            '''判断期望和实际结果'''
            self.assertEqual(eval(ExpectedResult),res.json())
            data_dict['TestResult']='PASS'
        except Exception as e:
            data_dict['TestResult']='FAILED'
            logger.error('出错啦错误是%s'%e)
            raise e
        finally:
            '''写回测试结果'''
            DoExcel(project_path.excel_path).write_back('test',CaseId+1,10,data_dict['TestResult'])
            logger.info('实际结果是{}，测试结果是：{}'.format(res.text,data_dict['TestResult']))
